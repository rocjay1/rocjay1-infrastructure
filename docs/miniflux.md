...
- **terraform/**: Cloudflare DNS, Zero Trust Tunnel, and Google Compute Engine provisioning via Terraform.
- **[ansible](ansible.md)**: Host configuration and Docker-based application deployment rules.
...
- Required sensitive variables: `uptime_check_secret`
- Optional overrides: `account_id`, `zone_id`, `zone_name`, `billing_account`, and `alert_email` (defaults are provided in `variables.tf`)
